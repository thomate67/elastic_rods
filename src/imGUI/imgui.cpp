...
void ImGui::ItemSize(const ImVec2& size, float text_offset_y)
{
	...
	const ImVec2 line_size = ImMax(window->DC.CurrentLineSize, size);
	...
	if (!window->DC.CurrentLayout || window->DC.CurrentLayout->Type == ImGuiLayoutType_Vertical)
	{
		...
		window->DC.CursorPos = ImVec2(
			...
			(float)(int)(window->DC.CursorPos.y + line_size.y + g.Style.ItemSpacing.y));
		window->DC.CursorMaxPos.x = ImMax(window->DC.CursorMaxPos.x, window->DC.CursorPosPrevLine.x);
		window->DC.CursorMaxPos.y = ImMax(window->DC.CursorMaxPos.y, window->DC.CursorPos.y - g.Style.ItemSpacing.y);

		window->DC.PrevLineSize = ImVec2(window->DC.CurrentLineSize.x, line_size.y);
	}
	else
	{
		// Move cursor to next horizontal line, x to right, y to top of last line.
		window->DC.CursorPos = ImVec2(
			(float)(int)(window->DC.CursorPos.x + line_size.x + g.Style.ItemSpacing.x),
			(float)(int)(window->DC.CursorPosPrevLine.y));
		window->DC.CursorMaxPos.x = ImMax(window->DC.CursorMaxPos.x, window->DC.CursorPosPrevLine.x);
		window->DC.CursorMaxPos.y = ImMax(window->DC.CursorMaxPos.y, window->DC.CursorPos.y + line_size.y + g.Style.ItemSpacing.y);

		window->DC.PrevLineSize = ImVec2(line_size.x, window->DC.CurrentLineSize.y);
	}
...
void ImGui::OpenPopupEx(ImGuiID id, ImVec2 pos)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* parent_window = g.CurrentWindow;
	int current_stack_size = g.CurrentPopupStack.Size;
...
void ImGui::OpenPopup(const char* str_id, ImVec2 mousepos)
{
	ImGuiContext& g = *GImGui;
	OpenPopupEx(g.CurrentWindow->GetID(str_id), mousepos);
}
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
	...
	if (first_begin_of_the_frame)
	{
		...
		if (window_just_activated_by_user)
		{
			...
			//TODO: fix opening window position, window is opened in screen coodinates but using grid coordinates 
		}
...
{ ImGuiDataType_Float, 1, (ImU32)IM_OFFSETOF(ImGuiStyle, AntiAliasFringeScale) }, // ImGuiStyleVar_AAFringeScale
};
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
	...
	const float text_base_offset_y = ImMax(padding.y, window->DC.CurrentLineTextBaseOffset - padding.y); // Latch before ItemSize changes it
...
static void EndLayout(ImGuiLayoutType type)
{
	...
	window->DC.CursorMaxPos = layout->StartPos;
	ImGui::ItemSize(new_size);
	ImGui::ItemAdd(ImRect(layout->StartPos, layout->StartPos + measured_size), 0);
	...
	//ImGui::GetCurrentWindow()->DrawList->AddRect(layout->StartPos, layout->StartPos + measured_size, 0xFF00FF00);   // Debug
...
static ImVec2 CalculateLayoutSize(ImGuiLayout& layout, bool collapse_springs)
{
	...
	if (layout.Type == ImGuiLayoutType_Vertical)
	{
		for (int i = 0; i < layout.Items.size(); ++i)
		{
			...
			{
				bounds.y += floorf(item.SpringSpacing);
	...
	else
	{
		for (int i = 0; i < layout.Items.size(); ++i)
		{
			...
			{
				bounds.x += floorf(item.SpringSpacing);
...
static void BalanceLayoutSprings(ImGuiLayout& layout)
{
	...
	for (int i = 0; i < layout.Items.size(); ++i)
	{
		...
		if (free_space > 0.0f && total_spring_weight > 0.0f)
		{
			...
			float span_end = floorf((i == last_spring_item_index) ? free_space : (free_space * next_weight / total_spring_weight));
		...
		// If spring changed its size, fixup positioning of following
		// items to avoid one frame visual bugs.
		if (last_spring_size != item.SpringSize)
		{
			...
			{
				ImGuiLayoutItem& translatedItem = layout.Items[j];

				TranslateLayoutItem(translatedItem, offset);

				translatedItem.MeasuredBounds.Min += offset;
				translatedItem.MeasuredBounds.Max += offset;
			}
		}
	}
}
...
static ImGuiLayoutItem* GenerateLayoutItem(ImGuiLayout& layout, ImGuiLayoutItemType type)
{
	...
	GImGui->CurrentWindow->DC.CurrentLayoutItem = &layout.Items[layout.CurrentItemIndex];
...
static float CalculateLayoutItemAlignmentOffset(ImGuiLayout& layout, ImGuiLayoutItem& item)
{
	...
	float align_offset = floorf(item.CurrentAlign * (layout_extent - item_extent));
...
static void BeginLayoutItem(ImGuiLayout& layout)
{
	...
	if (item.CurrentAlign < 0.0f)
		item.CurrentAlign = ImClamp(ImGui::GetStyle().LayoutAlign, 0.0f, 1.0f);
	...
	if (item.CurrentAlign > 0.0f)
	{
		if (layout.Type == ImGuiLayoutType_Horizontal)
		{
			float new_position = ImGui::GetCursorPosY() + item.CurrentAlignOffset;
			ImGui::SetCursorPosY(new_position);
		}
		else
		{
			float new_position = ImGui::GetCursorPosX() + item.CurrentAlignOffset;
	...
	item.MeasuredBounds.Min = ImGui::GetCursorScreenPos();
	item.MeasuredBounds.Max = item.MeasuredBounds.Min;

	item.VertexIndexBegin = ImGui::GetWindowDrawList()->_VtxCurrentIdx;
...
static void EndLayoutItem(ImGuiLayout& layout)
{
	...
	if (layout.Type == ImGuiLayoutType_Horizontal)
		ImGui::SetCursorScreenPos(ImVec2(ImGui::GetCursorScreenPos().x, layout.StartPos.y));
	else
		ImGui::SetCursorScreenPos(ImVec2(layout.StartPos.x, ImGui::GetCursorScreenPos().y));
...
static void AddLayoutSpring(ImGuiLayout& layout, float weight, float spacing)
{
	...
	if (layout.Type == ImGuiLayoutType_Horizontal)
		ImGui::SetCursorScreenPos(ImVec2(previous_item->MeasuredBounds.Max.x, ImGui::GetCursorScreenPos().y));
	else
		ImGui::SetCursorScreenPos(ImVec2(ImGui::GetCursorScreenPos().x, previous_item->MeasuredBounds.Max.y));

	previous_item = NULL; // may be invalid after call to GetLayoutItemAs()

	EndLayoutItem(layout);

	ImGuiLayoutItem* spring_item = GenerateLayoutItem(layout, ImGuiLayoutItemType_Spring);

	spring_item->MeasuredBounds.Min = ImGui::GetCursorScreenPos();
	spring_item->MeasuredBounds.Max = spring_item->MeasuredBounds.Min;
	...
	if (spring_item->SpringSize > 0 || spacing > 0)
	{
...
void ImGui::NextColumn()
{
	...
	if (++columns->Current < columns->Count)
	{
		...
		// FIXME-LAYOUT: probably we should select relevant axis basing on current layout
		window->DC.ColumnsOffset = ImVec2(GetColumnOffset(columns->Current), 0.0f) - window->DC.Indent + ImVec2(g.Style.ItemSpacing.x, 0.0f);
...
{
	// FIXME-LAYOUT: Select axis according to current layout
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = GetCurrentWindow();
